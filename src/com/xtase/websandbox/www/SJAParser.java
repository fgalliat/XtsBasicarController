...
/**
 * Parser de fichier '.SJA'<BR>
 * <B>NB:</B> Les fichiers SJA sont des fichiers de script DynamicJava coted
 * serveur
 * 
 * @author Franck Galliat
 */

public class SJAParser {

	/** le contenu Html de la page (celui qui sera affiched) */
	String pageContentHtml = "";
	/** le Parser Javascript - voir (./lib/js/JS.JAR) */
	DynamicJavaParser dynJavaParser = null;
	/** activer le debugage ? */
	boolean trace = !true;
	/**
	 * le type mime du document retourned apres execution (par defaut
	 * 'text/html')
	 */
	 ...
	 * 
	 * @param kernel
	 *            l'instance de Kernel (utilised pour avoir acces au Controllers
	 *            & aux Providers)
	 * @param uri
	 *            l'URL du script
	 * @param server
	 *            l'instance du WebServer (utilised pour le controle des
	 *            Sessions)
	 * @param req
	 *            la requette Http associede
	 * @throws Exception
	 */
	public void exec(String uri, WebServer server, HttpEmptyServer.HttpReq req)
			throws Exception {
		...
		// long t0exec, t1exec;
		// t0exec = now();
		dynJavaParser = new DynamicJavaParser(null);
		dynJavaParser.addReference("document", this);
		dynJavaParser.addReference("HttpRequest", req);
		dynJavaParser.addReference("WebServer", server);

		// jsParser.addReference("context",
		// AndroWebServerActivity.getInstance());
		...
		if (trace) {
			try {
				PrintStream sourceOut = new PrintStream(new FileOutputStream(
						"trace.log.txt"));
		...
		try {
			dynJavaParser.evaluate(sourceCode);
		} catch (Exception ex) {
			pageContentHtml += "<DIV style='background-color:red;color:white;font-family=verdana,helvetica,sans-serif'>"
					+ ex.toString() + "</DIV>";
			ex.printStackTrace();
		}
		dynJavaParser.dispose();
		dynJavaParser = null;
		System.gc();
		// t1exec = now();
		...
		req.pipe.sendResponse(HttpEmptyServer.HTTP_OK, extraHead,
				pageContentHtml, mimeType);
	}
	...
	public String getPageCode(String content, String uri) throws Exception {
		...
		while (true) {
			...
			if (cleanFrag.length() > 0) {
				...
				sourceCode += replaceBy(
						replaceBy(
								replaceBy(replaceBy(cleanFrag, "\\", "\\\\"),
										"\r", " "), "\n", "\\n"), "\"", "\\\"");
				sourceCode += "\");\n";
			}
			...
			if (source.startsWith("=")) { // <?= ... ?>
				...
				while ((incStart = source.indexOf("#include:")) > -1) {
					...
					between = source
							.substring(incStart, incStop + "#".length());
					between = between.substring(between.indexOf(":") + 1,
							between.length() - "#".length()).trim();

					// String content =
					// server.getDocumentRoot().getContent(uri);
					String includeUri = getParentPath(uri) + between;
					String tmp = server.getDocumentRoot()
							.getContent(includeUri);
					if (tmp == null)
						throw new Exception("Include Introuvable ("
								+ includeUri + ")");
	...
	public static String replaceBy(String str, String oldE, String newE) {
		...
		//
		// // indexOf etant implemented a peut pres pareil, il ne sert a rien de
		// l'utiliser pour booster le code !
		// String retBuffer = "";
		// char[] toReplace = oldE.toCharArray();
		// //char[] byReplace = newE.toCharArray();
		//
		//
		// // new !!!!! trash - cf bug dur dernier char s'il correspond au
		// premier char de la recherche
		// char[] support = (str+ ( toReplace[0] == '.' ? "*" : "." )
		// ).toCharArray();
		//
		//
		// int found = 0;
