...
import com.xtase.xtsubasic.MiniTelnetBridge;
...
public class Main {
    ...
    public static void connectToXtsuBasicViaWifi(String ip) {
        ...
        new Thread() {
            public void run() {
                try {
                    new MiniTelnetBridge().doWork(1123);
                } catch (Exception ex) {
                    currentBridge = null;
                    // notify user !
                    ex.printStackTrace();
        ...
        new Thread() {
            public void run() {
                try {
                    currentBridge = ip;
                    // to telnet Bridge -> Xts-uBASIC WIFI telnetd
                    WebSocketToTelnet.main(new String[]{"9878", "1123", ip});
                    // notify user !
